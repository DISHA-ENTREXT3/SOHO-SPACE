...
test.describe('Security & Input Validation', () => {
  ...
  test('Support Chatbot should sanitize XSS payloads', async ({ page }) => {
    ...
    const messageInput = page.locator('textarea[placeholder*="Describe your growth request"]');
    await messageInput.fill(xssPayload);
    
    // Fill email
    const emailInput = page.locator('input[placeholder="alias@nexus.io"]');
    await emailInput.fill('test@example.com');

    // Wait for button to be enabled
    const submitButton = page.locator('button:has-text("Initiate Transmission")');
